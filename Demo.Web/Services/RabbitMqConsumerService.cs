...
public class RabbitMqConsumerService : BackgroundService
{
    ...
    private void InitializeRabbitMq()
    {
        try
        {
            ...
            {
                _logger.LogWarning("RabbitMQ credentials ontbreken in configuratie. Consumer wordt uitgeschakeld.");
                _connection = null;
                _channel = null;
                return;
            }
            ...
            _logger.LogInformation(
                "RabbitMQ Consumer connecting to {HostName}:{Port} vhost '{VirtualHost}' as user '{UserName}'",
                hostName,
                port,
                virtualHost,
                userName);
